...
import com.xinshuwu.util.AnimationUtil;
import com.xinshuwu.util.ThemeUtil;
...
public class TabLayout extends HorizontalScrollView {
    ...
    private SlidingTabStrip mTabStrip = null;
    ...
    private int mRequestedTabMinWidth = 0;
    private int mRequestedTabMaxWidth = 0;
    private int mScrollableTabMinWidth = 0;
    ...
    public TabLayout(Context context, int mRequestedTabMinWidth) {
        this(context, null);
    }
    ...
    public TabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        new ThemeUtil().checkAppCompatTheme(context);
    ...
    private void addTabFromItemView(@NonNull TabItem item) {
        ...
        if (item.getMText() != null) {
            tab.setText(item.getMText());
        }
        if (item.getMIcon() != null) {
            tab.setIcon(item.getMIcon());
        }
        if (item.getMCustomLayout() != 0) {
            tab.setCustomView(item.getMCustomLayout());
        }
    ...
    private void ensureScrollAnimator() {
        if (mScrollAnimator == null) {
            ...
            mScrollAnimator.setInterpolator(new AnimationUtil().getFAST_OUT_SLOW_IN_INTERPOLATOR());
    ...
    private class SlidingTabStrip extends LinearLayout {
            ...
            if (selectedTitle != null && selectedTitle.getWidth() > 0) {
                ...
                if (mSelectionOffset > 0f && mSelectedPosition < getChildCount() - 1) {
                    ...
                    if (mSelectionOffset <= 0.5f) {
                        left = selectedTitle.getLeft();
                        right =new AnimationUtil().lerp(right, nextTitle.getRight(), mSelectionOffset * 2);
                    } else {
                        left = new AnimationUtil().lerp(left, nextTitle.getLeft(), (mSelectionOffset - 0.5f) * 2);
        ...
        void animateIndicatorToPosition(final int position, int duration) {
            ...
            if (startLeft != targetLeft || startRight != targetRight) {
                ...
                animator.setInterpolator(new AnimationUtil().getFAST_OUT_SLOW_IN_INTERPOLATOR());
                ...
                animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                    ...
                    public void onAnimationUpdate(ValueAnimator animator) {
                        ...
                        if (mSelectedPosition < position) {
                            if (fraction <= 0.5f) {
                                left = startLeft;
                                right = new AnimationUtil().lerp(startRight, targetRight, fraction * 2);
                            } else {
                                left = new AnimationUtil().lerp(startLeft, targetLeft, (fraction - 0.5f) * 2);
                                right = targetRight;
                            }
                        } else {
                            if (fraction <= 0.5f) {
                                left = new AnimationUtil().lerp(startLeft, targetLeft, fraction * 2);
                                right = startRight;
                            } else {
                                left = targetLeft;
                                right = new AnimationUtil().lerp(startRight, targetRight, (fraction - 0.5f) * 2);
                            }
                        }
                        setIndicatorPosition(left, right);
                    }
